...
#### test avec le changement de voie, route infinie avec 2 voies, avec 8 véhicules
...
vehicles = {'vehicle':[1,2], 'classe vehicule':['HD','HD'],'type vehicule':['VL','VL'],'aggressivity 0':[50,50], 'aggressivity':[50,50], 'reaction time':[1,0.5], 'courtesy':[50,50], 'respect code de la route':[50,50], 'road': [1, 1], 'lane': [1, 2],'road turn':[3,3],'lane turn':[1,1], 'position turn':[1,1]}
vehicles=pd.DataFrame(data=vehicles)
Network={'road':[1,1],'lane':[1,2],'vitesse limite':[30,30]}
Network=pd.DataFrame(data=Network)
trajectories0={'vehicle':[1], 'time':[0], 'acceleration':[0], 'vitesse':[25], 'position':[0], 'road':[1], 'lane':[1] }
trajectories0=pd.DataFrame(data=trajectories0)
for j in arange(t_simu_deb-10, t_simu_fin+1, 0.1):
    trajectories0 = pd.DataFrame(np.array([[1,round(j,1), 0, 25, 250+j*25, 1, 2]]),columns=["vehicle", "time", "acceleration", "vitesse", "position", "road", "lane" ]).append(trajectories0, ignore_index=True)
