# -*- coding: utf-8 -*-
"""
Created on Mon Jun  3 09:39:01 2019
...
def general(ego_vehicle, Network, vehicles, t_simu_deb, t_simu_fin,tp, trajectories) :   
    ...
    t=t_simu_deb
    tr_bis= ego_vehicle.iloc[0]['reaction time'] #à modifier plus tard por avoir un nombre entier de pas de temps dans tr_bis
    a_max=2   
...
###### recherche des états (position, vitesse, accélération) précédents (en t-tp et t-tr) de ego_vehicle    
        
        a_moins_tr=trajectories[(trajectories['vehicle']==ego_vehicle.iloc[0]['vehicle'])&(trajectories['time']==round(t-tr_bis,1))]
        a_moins_tr=a_moins_tr.iloc[0]['acceleration']
        v_moins_tr=trajectories[(trajectories['vehicle']==ego_vehicle.iloc[0]['vehicle'])&(trajectories['time']==round(t-tr_bis,1))]
        v_moins_tr=v_moins_tr.iloc[0]['vitesse']
        x_moins_tr=trajectories[(trajectories['vehicle']==ego_vehicle.iloc[0]['vehicle'])&(trajectories['time']==round(t-tr_bis,1))]
        x_moins_tr=x_moins_tr.iloc[0]['position']
...
###### estimation des voies possibles (i.e sur lesquelles ego_vehicle peut aller) dans la route sur laquelle se trouve ego_vehicle    
        ...
        A=environment.adjoining_road(ego_vehicle.iloc[0]['road'],ego_vehicle.iloc[0]['lane'],Network)

####### premier cas: ego_vehicle peut seulement rester dans sa voie (il n'y a qu'une voie sur la route où il y a des restrictions de circulation sur les autres voies)
        ...
        if len(A)==1:
            ...
            a_mic=state.HDM(ego_vehicle, leads, Network,trajectories, t, tr_bis) # on calcule la nouvelle accélération donnée par la loi de poursuite HDM
            a=state.perception_threshold(a_prece,a_mic) # on considère cette nouvelle accélération comme l'accélération de ego_vehicle en t seulement si le seuil de perception est dépassé,sinon l'accélération est l'accélération précédente
...
####### deuxième cas: ego_vehicle a le choix entre plusieurs voies:
            ...
            Af_avec=[]
            Ae=[]
            ego_vehicle.iloc[0]['aggressivity']=lane_change.aggressivity(Ag0, cv,x_prece, road, ego_vehicle.iloc[0]['position turn'],ego_vehicle.iloc[0]['road turn'], v_prece)# on récupère l'aggréssivité du véhicule
            Leaders=[]
            Followers=[]
    ...
    #### pour chaque voie on va calculer les différentes accélérations (accélérations du vehicule follower avec et sans la présence de ego_vehicle dans la voie et accélération de ego_vehicle ) 
            for i in range(len(A)):
                ...
                follow=environment.following_vehicle(x_prece, ego_vehicle.iloc[0]['road'], A[i], vehicles, trajectories,t)
                leads=environment.leading_vehicles(x_prece,ego_vehicle.iloc[0]['road'],A[i],ego_vehicle.iloc[0]['classe vehicule'],vehicles,trajectories, t) #je pense qu'il faut regarder x au pas de temps précédent... à voir, je sais pas pourquoi il veut pas appeler la fonction avec environment
                ...
                if follow==[]: #si il n'y a pas de follower (i.e. il n'y a pas d'impact sur le véhicule follower si ego_vehicle change de voie ou reste dans la voie)
                    af_sans=0
                    af_avec=0
                elif follow[0]==ego_vehicle.iloc[0]['vehicle']:#parce qu'il y a un problème avec ma fonction follower qui donne parfois ego_vehicle comme son propre follower, pour l'instant, quand il y a cette erreur, je considère qu'il n'y a pas de vehicle follower
                    af_sans=0
                    af_avec=0
                    ...
                    af_sans=state.HDM(vehicles[vehicles['vehicle']==follow],leads,Network, trajectories, t, tr_bis)# (dans la loi HDM on ne considère que l'impact d'un leader parce qu'on va dire que mon conducteur ne va pas anticiper avec plusieurs véhicules pour changer de voie)
                    af_avec=state.HDM(vehicles[vehicles['vehicle']==follow],[ego_vehicle.iloc[0]['vehicle']], Network,trajectories, t, tr_bis) #le leader de follower devient ego_vehicle quand ego_vehicle est dans la voie
                

                #### on calcule ensuite l'accélération de ego_vehicle dans la voie considérée
                ae=state.HDM(ego_vehicle,leads, Network, trajectories, t, tr_bis)
                ae2=state.perception_threshold(a_prece,ae)# on passe par le perception threshold seulement pour ego_vehicle parce qu'on suppose que la personne ne regarde pas constamment l'accélération des autres et ne la calcule que ponctuellement quand il veut changer de voie
    ...
    ##### pour chaque voie on va ensuite évaluer le safety criterion (grace aux accélérations calculées précédemment) et déterminer les voies dans lesquelles le véhicule peut aller:
            ...
            if cv=='HD' or (cv=='AV' and safe_stay==True): #si on a un HD ou on a un AV pour lequel le critère de sécurité est respecté si on reste dans la même voie: on va juste chercher les autres voies possibles
    ...
    ##### parmi les voies possibles (i.e. voies "safe") on choisit et on attribue sa nouvelle voie à ego_vehicle            
            if len(Possible_lane) == 0: # si aucune voie n'est "safe" pour le véhicule autonome, ego_vehicle va choisir la voie la "moins dangereuse"
                sc.append(Ae[-1]+Af_avec[-1])
                ind=sc.index(max(sc))
                lanea=sc[ind]
                a=Ae[ind] # on attribue la nouvelle accélération de ego_vehicle REVOIR!!!!
            
                
            elif len(Possible_lane)==1 : # si il n'y a qu'une seule voie "safe", ego_vehicle choisit cette voie
                lanea=Possible_lane[0]
                a=Ae[0] # on attribue la nouvelle accélération de ego_vehicle
                ...
                for k in range(len(Possible_lane)-1):
                    ...
                    O=lane_change.utility_overtaking(xl1,xl2,lane,lane2,x_prece, R)
                    T=lane_change.utility_truck(tv1,tv2, cv)
                    C=lane_change.utility_courtesy(Af_sans[k],Af_avec[k],Af_sans[-1], Af_avec[-1], ego_vehicle.iloc[0]['courtesy'])
                    Tu=lane_change.utility_turn(x_prece, ego_vehicle.iloc[0]['position turn'],lane, ego_vehicle.iloc[0]['lane turn'], Possible_lane[k], ego_vehicle.iloc[0]['road turn'], road, cv, v_prece)
